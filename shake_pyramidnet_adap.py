...
class ShakePyramidNet(nn.Module):
    ...
    def __init__(self, depth=110, alpha=270, label=10):
        ...
        self.convs0 =  nn.ModuleList([nn.utils.spectral_norm(nn.Conv2d(3, 8, kernel_size=1, stride=1, padding=0, bias=False)) for _ in range(32*32)])
        self.bns0 = nn.ModuleList([nn.BatchNorm2d(8) for _ in range(32*32)])
        self.convs1 =  nn.ModuleList([nn.utils.spectral_norm(nn.Conv2d(8, 16, kernel_size=1, stride=1, padding=0, bias=False)) for _ in range(32*32)])
        self.bns1 = nn.ModuleList([nn.BatchNorm2d(16) for _ in range(32*32)])
        self.ll = nn.LeakyReLU(0.2)

        self.convs0 =  nn.utils.spectral_norm(nn.Conv2d(3, 8, kernel_size=1, stride=1, padding=0, bias=False)) 
        self.bns0 = nn.BatchNorm2d(8) 
        self.convs1 =  nn.utils.spectral_norm(nn.Conv2d(8, 16, kernel_size=1, stride=1, padding=0, bias=False)) 
    ...
    def forward(self, x):
#        x = self.ll(self.bns0(self.convs0(x)))#.view(-1,8,1,1)
  #     x = self.ll(self.bns1(self.convs1(x)))
